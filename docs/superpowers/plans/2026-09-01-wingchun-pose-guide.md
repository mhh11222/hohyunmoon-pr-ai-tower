...
- [x] 독립 리뷰(엔지니어링·디자인 19건 + 코드리뷰 7건) → 반영: WASM CSP, 스크롤 납치, 유휴 렌더 생략, 마지막 자세 전환, 빈 데이터 가드, 카메라 수명주기, 프레임 스텝 경계, 키보드/접근성, 터치 크기, 책 사진 레이아웃, 각도 호 버퍼 재사용, MediaPipe 자체 호스팅
- [x] 카메라 end-to-end: 시범 영상을 가짜 카메라로 넣어 인식·일치도·재시작(0.8초) 확인, 외부 호스트 요청 0
